...
func TestDeadPoolReaper(t *testing.T) {
	...
	reaper := newDeadPoolReaper(ns, pool, []string{}, noopLogger{})
...
func TestDeadPoolReaperNoHeartbeat(t *testing.T) {
	...
	reaper := newDeadPoolReaper(ns, pool, []string{"type1"}, noopLogger{})
	deadPools, err := reaper.findDeadPools()
	assert.NoError(t, err)
...
func TestDeadPoolReaperWithWorkerPools(t *testing.T) {
	...
	wp.deadPoolReaper = newDeadPoolReaper(wp.namespace, wp.pool, []string{"job1"}, noopLogger{})
...
func TestDeadPoolReaperCleanStaleLocks(t *testing.T) {
	...
	reaper := newDeadPoolReaper(ns, pool, jobNames, noopLogger{})
